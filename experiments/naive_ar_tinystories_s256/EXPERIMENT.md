# naive_ar_tinystories_s256 — Experiment Design

**Slides:** `slides/jun25_2026/slides.md` — "Naive AR Baseline", **Max Seq Len = 256**.

## Hypothesis
Standard non-geometry small-DiT baselines trained on TinyStories at seq-256 are the
reference points against which the geometry flows (S/E/H-FLM) and advanced tricks are
compared. Two cells:
- **`ar`** — causal autoregressive. Valid PPL is a *true* AR PPL.
- **`mdlm`** — masked (absorbing) discrete diffusion (MDLM). Valid PPL is a
  denoising-ELBO bound, NOT comparable to AR PPL; compare cells on GenPPL + entropy.

## Design
- 2 runs. Identical small DiT (width 768, depth 12, heads 12), `model=small`.
  - `ar`:   `algo=ar` (causal attention), greedy decoding eval.
  - `mdlm`: `algo=mdlm` (absorbing diffusion, `subs` parameterization, no time
    conditioning), ancestral sampling eval (256 steps).
- 30k steps, global batch 512, **seq 256**, bf16, EMA 0.9999, AdamW (lr 3e-4, wd 0,
  betas (0.9,0.999), eps 1e-8, grad-clip 1.0).
- **Checkpoints every 5k steps, all retained** (`SAVE_TOPK=-1`).

## GPU allocation
- 1 job per cell, `gpu:4` on `thickstun,desa` (exclude desa-compute-01). `PER_GPU_BS=32`
  (accum = 512/(4×32) = 4). Train→eval in one SLURM job; idempotent/resumable.

## Expected wall-clock
- ~16.4 GPU-hr of compute on RTX 6000 Ada at bs=32; with 4 GPUs ≈ **4–5 hr** train
  + ~0.5 hr eval per cell. Delivered in **< 1 day**.

## Outputs
`outputs/naive_ar_tinystories_s256/{ar,mdlm}/` → checkpoints/, eval/ppl.json,
eval/samples_genppl.json.
Report: `experiments/naive_ar_tinystories_s256/RESULTS.md` (via `report.py`).
