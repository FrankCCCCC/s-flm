#!/usr/bin/env python
"""hflm_sweep_tinystories (slides jun25_2026) — H-FLM embedding-init x init-noise grid.
...
PER_GPU_BS=8, accum auto = 64), seq 1024, bf16, EMA 0.9999, AdamW lr 3e-4, noise=log-linear.
...
EXP = f'{REPO}/experiments/hflm_sweep_tinystories'
LOGS = f'{EXP}/logs'
OUT = f'{REPO}/outputs/hflm_sweep_tinystories'
...
def job_body(init, std, pc):
    ...
    return textwrap.dedent(f'''\
        ...
        {init_env(init, std)} PRIOR_COV={pc} RHO_MAX={RHO_MAX} OUTPUT_DIR={tdir} RUN_NAME=hflm_{tag} DEVICES=1 PER_GPU_BS=8 \\
            bash scripts/train/tinystories/hlfm.sh
        echo "[$(date)] EVAL {tag}"
        PRIOR_COV={pc} RHO_MAX={RHO_MAX} CKPT_PATH={tdir}/checkpoints/last.ckpt OUTPUT_DIR={edir} DEVICES=1 \\
...
def main():
    ...
    print(f'hflm_sweep_tinystories: {len(cells)} cells '
          f'({len(INITS)} init x {len(PRIOR_COVS)} prior_cov)')
    if args.dry_run:
        for i, s, pc in cells:
            print('  hflm_' + tag_of(i, s, pc))
        print('\n--- example body ---\n' + job_body(*cells[0]))
        return
