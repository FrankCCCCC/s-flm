#!/usr/bin/env python
"""naive_ar_tinystories (slides jun25_2026) — Naive AR baseline.

Single run: causal small DiT (768/12/12), 30k steps, batch 512, seq 1024, bf16,
EMA 0.9999, AdamW (config defaults already match the slide). Eval = greedy decoding.

This sweep only ORCHESTRATES: it calls the single-run shared scripts
...
EXP = f'{REPO}/experiments/naive_ar_tinystories'
LOGS = f'{EXP}/logs'
OUT = f'{REPO}/outputs/naive_ar_tinystories'
DEVICES = 4
PER_GPU_BS = 8
