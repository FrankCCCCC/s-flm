...
Documents the exact checkpoint path and hyperparameters behind each figure.
Figures are in `sudoku_hard/hflm_K*/` (e.g. `hflm_K0.5/K0.5.png`). For the
analysis/findings see [`../RESULT.md`](../RESULT.md)
...
`hflm_curv_overlay{,_log}.png` (in `sudoku_hard/`) = one curve per curvature at
its final step (all 20K).
