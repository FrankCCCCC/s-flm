#!/bin/bash
# Eval for H-FLM trained with truncated (non-adaptive) noise schedule. The
# noise config must mirror training (the truncated schedule drives the
# sampling trajectory). GAUSS_CURV / INIT / INIT_STD / ALPHA_MAX must match
# the training run. ALPHA_MAX default = alpha_star_hyperbolic(12, 512) =
# 0.624 for INIT=hyperbolic, K=-1 (noise_schedules.py); other geometries:
# experiments/trunc_ada_sudoku/alpha_star_numeric.py.
set -euo pipefail
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1

REPO_ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
CKPT_PATH="${CKPT_PATH:?set CKPT_PATH to the trained HFLM truncated sudoku checkpoint}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/eval_runs/sudoku/hflm_truncated_${DIFFICULTY}}"
