#!/bin/bash
# EFLM rescale + AUTONOMOUS clock — eval ONE TinyStories checkpoint: valid PPL
# (ppl_eval) + GenPPL (sample_eval). RHO and TAU_MAX must match training; the
# uniform t grid is uniform in tau, which is what makes every Euler step advance
# the flow by the same amount.
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/tinystories/eval/eflm_rescale_auto_adaptive}"
DEVICES="${DEVICES:-1}"
EVAL_BS="${EVAL_BS:-16}"
STEPS="${STEPS:-180}"
