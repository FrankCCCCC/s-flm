#!/bin/bash
# SFM truncated — eval ONE TinyStories checkpoint: valid PPL (ppl_eval) + GenPPL (sample_eval).
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/tinystories/eval/sfm_truncated}"
DEVICES="${DEVICES:-1}"
EVAL_BS="${EVAL_BS:-16}"
STEPS="${STEPS:-180}"
