#!/bin/bash
# SimpFLM with fixed embedding norm R (rho_min = rho_max = RHO) on the AUTONOMOUS
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/tinystories/simpflm_rescale_auto_truncation}"
RUN_NAME="${RUN_NAME:-simpflm_rescale_auto_truncation}"
...
python -u -m main \
    ...
    wandb.project=tinystories-simpflm \
