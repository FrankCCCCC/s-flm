...
# adaptive noise schedule. Single TinyStories training run. ALPHA_MAX should
# be the R-dependent Eq. 17 bound alpha_star_euclidean(V=50257,
# embed_norm=RHO) (noise_schedules.py); the default 0.840 is the RHO=1 value.
set -euo pipefail
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1

REPO_ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
CACHE_DIR="${CACHE_DIR:-${REPO_ROOT}/data_cache}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/tinystories/eflm_rescale_truncated_adaptive}"
RUN_NAME="${RUN_NAME:-eflm_rescale_truncated_adaptive}"
